...
def build_tags(
    sub_cat: str, category: str, material: list, style: str, season: str, gender: str
) -> str:
    tags = [
...
def generate_products(cat_ids: dict) -> list[dict]:
    ...
    for cat_name, count in DISTRIBUTION.items():
        ...
        for _ in range(count):
            ...
            colors = (
                random.sample(info["colors"], k=min(2, len(info["colors"])))
                if info["colors"]
                else []
            )
            sizes = (
                random.sample(SIZE_RANGES, k=random.randint(2, 5))
                if cat_name == "Fashion & Apparel"
                else []
            )
